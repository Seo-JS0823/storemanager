...
const endDateEl = document.querySelector('#enddate'); 		// 검색 마지막일칸
	...
	function ipgoRender(page) {
		...
		const searchEvent3Ell = document.querySelector('#searchEvent3');
		...
		if(startDate && endDate) {
			urlll = `/In/list?start=\${startDate}&end=\${endDate}`;
		}
		
		if(searchEvent1Ell.checked) {
			urlll = 'in/searchItem/' + keyword;
			if(startDate && endDate) {
				urlll += `?start=\${startDate}&end=\${endDate}`;
			}
		} else if (searchEvent2Ell.checked) {
			urlll = 'in/searchCompany/' + keyword;
			if(startDate && endDate) {
				urlll += `?start=\${startDate}&end=\${endDate}`;
			}
		} else if (searchEvent3Ell.checked) {
			urlll = 'in/searchPrice/' + keyword;
			if(startDate && endDate) {
				urlll += `?start=\${startDate}&end=\${endDate}`;
			}			
		}
